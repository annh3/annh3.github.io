...
Starting with the expression for the KL divergence between the learned model $q_{\phi}(z|x)$ and the true posterior $p(z|x)$, we can derive the evidence lower bound (ELBO) which is a lower bound on $p(x)$ and an objective that through maximizing we can obtain an estimate for $p(x)$, converting the inference problem into an optimization problem. 
