...
algorithm based on Jensen's inequality. This post will be concerned with deriving the EM algorithm from first principles, exploring use cases, such as optimizing mixtures of Gaussians, relating it to variational inference using the reparameterization trick, and showing how it is conceptually similar to Trust Region Policy Optimization. I am indebted to the Stanford CS229 material, [this](https://arxiv.org/pdf/2104.06159.pdf) paper, and these wonderful, wonderful [notes](http://cs229.stanford.edu/notes2020spring/cs229-notes8.pdf) by Tengyu Ma. 
...
Recall previously that we had to make a choice for the sampling distribution of the ELBO bound, $$Q(z)$$. Specifically, choosing $$Q(z) = p(z|x;\theta)$$ gave us a tight ELBO bound to optimize against. Although this worked for Mixture of Gaussians, in many complex models it's intractable to compute $$p(z|x;\theta)$$ exactly. Since ELBO works for arbitrary $$Q$$, we instead use an approximation of the true posterior distribution.
...
$$ \begin{equation}
ELBO(\phi, \psi, \theta) = \sum_{i=1}^n \mathbb_{E}_{z^{(i)} \sim Q_{i}} \big[ \log \dfrac{p(x^{(i)},z^{(i)};\theta)}{Q_{i}(z^{(i)})} \big]
\end{equation} $$
...
$$ \begin{align*}
\mathbb_{E}_{z^{(i)} \sim Q_{i}} \big[ \log \dfrac{p(x^{(i)},z^{(i)};\theta)}{Q_{i}(z^{(i)})} \big] &= 
\mathbb_{E}_{\xi^{(i) \sim \mathcal{N}(0,1)} \big[ \log \dfrac{p(x^{(i)},q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)};\theta)}{Q_i{q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)}}} \big] \\
\end{align*} $$

And 

$$ \begin{align*}
\nabla_{\phi} \mathbb_{E}_{z^{(i)} \sim Q_{i}} \big[ \log \dfrac{p(x^{(i)},z^{(i)};\theta)}{Q_{i}(z^{(i)})} \big] &= 
\nabla_{\phi} \mathbb_{E}_{\xi^{(i) \sim \mathcal{N}(0,1)} \big[ \log \dfrac{p(x^{(i)},q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)};\theta)}{Q_i{q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)}}} \big] \\
&= \mathbb_{E}_{\xi^{(i) \sim \mathcal{N}(0,1)} \big[ \nabla_{\phi} \log \dfrac{p(x^{(i)},q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)};\theta)}{Q_i{q(x^{(i)};\phi) + v(x^{(i)};\psi) \odot \xi^{(i)}}} \big] \\
\end{align*} $$
...
Recall that $$ELBO(x;Q,\theta) = \sum_{z} Q(z) \log \frac{p(x,z;\theta)}{Q(z)}$$ and we can write

$$ \begin{align*}
ELBO(x;Q,\theta) &= \mathbb{E}_{z \sim Q}[\log p(x,z;\theta)] - \mathbb{E}_{z \sim Q}[\log Q(z)] \\
&= \mathbb{E}_{z \sim Q}[\log p(x ;\theta)] - D_{KL}(Q \| p_{z\vertx}) \\
\end{align*} $$

(To-Do: verify) Which shows that...

#### Connection to TRPO

#### Resources

1. [icml slides](https://icml.cc/media/Slides/icml/2019/104(12-14-00)-12-14-20-5058-generalized_maj.pdf)
2. [MM](https://arxiv.org/pdf/1104.2203.pdf)
3. [MM again](https://www.stat.berkeley.edu/~aldous/Colloq/lange-talk.pdf)
4. [MM vs EM](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3755471/)
5. [TRPO](https://jonathan-hui.medium.com/rl-trust-region-policy-optimization-trpo-explained-a6ee04eeeee9)
