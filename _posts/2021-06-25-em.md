...
$$ \begin{align*}
\nabla_{\Sigma_{l}} \sum_{i=1}^n \sum_{j=1}^k w_{j}^{(i)} \log \dfrac{1}{(2 \pi)^{d/2} \vert \Sigma_{j} \vert^{1/2}} exp(\dfrac{\frac{-1}{2} (x^{(i)} - \mu_{j})^{T} \Sigma_{j}^{-1} (x^{(i)} - \mu_{j}) \cdot \sigma_{j}}{w_{j}^{(i)}})
\end{align*} $$
