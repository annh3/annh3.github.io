...
$$\begin{equation}
P[Q(a_{t}) > \hat{Q}(a_{t}) + u] \le exp(-2tu^2) = \dfrac{\delta}{t^2}
\end{equation}
$$

Where $$\delta$$ is a parameter and $$t$$ is the current timestep and $$n(a_{t})$$ is action selection count. We'll see later why setting the bound to be $$\dfrac{\delta}{t^2}$$ is useful.
...
For the last inequality we used $$ \sum_{t=1}^{\infty} t^{-2} = \dfrac{\pi^2}{6} < 2$$, which puts our analysis in the infinite horizon case.
...
$$\begin{align*}
\sum_{t=1}^{T} U_{t}(a_{t}) - Q(a_{t}) &= \sum_{t=1}^{T} \hat{Q}(a_{t}) + \sqrt{\frac{1}{n(a_{t})} \log(t^2/\delta)} - Q(a_{t}) \\
& \le 2 \sum_{t=1}^T \sqrt{\dfrac{1}{n_t({a_t})} \log \dfrac{t^2}{\delta}} \\
&= 2 \sqrt{\dfrac{1}{2} \log \dfrac{T^2}{\delta}} \sum_{i=1}^m \sum_{n=1}^{n_{t}(i)} \sqrt{\dfrac{1}{n}}\\
& \le 2 \sqrt{\dfrac{1}{2} \log \dfrac{T^2}{\delta}} \sum_{i=1}^m \sum_{n=1}^{T/m} \sqrt{\dfrac{1}{n}}\\
& \le  2 \sqrt{\dfrac{1}{2} \log \dfrac{T^2}{\delta}} \sum_{i=1}^m 2 \sqrt{T/m} \\
& \le 4 \sqrt{\dfrac{1}{2} \log \dfrac{T^2}{\delta} Tm}
\end{align*}$$
...
* People have taken inspiration from UCB algorithm in the deep reinforcement learning setting. The specific strategy is to make actions look more optimistic than they are, thereby encouraging exploration. [Here](https://arxiv.org/abs/1611.04717) is an example of an approach which hashes states.
