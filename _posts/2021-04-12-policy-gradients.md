...
Taking ratio of the old and the new log probabilities, in addition to being a way to measure the magnitude of policy change accross the update, serves another purpose. Notice that in the objective we are estimating the value for the policy with parameters $$\theta_{k+1}$$ but our state-action samples are collected from the policy with parameter $$\theta_{k}$$. Using $$\dfrac{\pi_{\theta}(a \vert s)}{\pi_{\theta_{k}(a \vert s)}$$ allows us to re-weight our samples so we approximate the expectation of the loss with respect to $$\theta_{k+1}$$. This common technique in RL is called importance sampling.
